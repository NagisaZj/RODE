...
class RODECombMAC:
    def __init__(self, scheme, groups, args):
        ...
        self.not_bi_opt = (not args.bi_opt)
    ...
    def forward(self, ep_batch, t, test_mode=False, t_env=None):
        ...
        if t % self.role_interval == 0:
            self.role_outputs = self.role_selector(agent_inputs, self.role_latent)  # [bs * n_agents, n_roles]
            self.selected_roles = self.role_selector.select_role(self.role_outputs, test_mode=test_mode, t_env=t_env).squeeze()
            # [bs * n_agents]
        ...
        # individual q values
        role_outputs_softmax = th.softmax(self.role_outputs, dim=-1)
        if self.not_bi_opt:
            role_outputs_softmax = role_outputs_softmax.detach()
        role_outputs_softmax = role_outputs_softmax.view(ep_batch.batch_size * self.n_agents, 1, -1)
        # [bs * n_agents, 1, n_roles]

        agent_outs = th.bmm(role_outputs_softmax, roles_q)
        # [bs * n_agents, 1, n_actions]

        return agent_outs.view(ep_batch.batch_size, self.n_agents, -1), self.role_outputs.view(ep_batch.batch_size, self.n_agents, -1)
