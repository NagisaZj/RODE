...
class RODERNNMAC:
    ...
    def load_models(self, path):
        role_action_spaces = th.load("{}/role_action_spaces.pt".format(path),
                                     map_location=lambda storage, loc: storage).to(self.args.device)
        self.n_roles = self.role_action_spaces.shape[0]
        self.role_action_spaces = th.ones(self.n_roles, self.n_actions).to(self.args.device)
        self.role_action_spaces[:, :role_action_spaces.shape[1]] = role_action_spaces
        print(self.role_action_spaces)
        ...
        self.action_repr = th.load("{}/action_repr.pt".format(path),
                                   map_location=lambda storage, loc: storage).to(self.args.device)
        self.action_repr = th.cat([self.action_repr, self.action_repr[:self.n_actions - self.action_repr.shape[0]]], dim=0)
